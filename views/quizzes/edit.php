...
<form action="/admin/quizzes/update" method="POST">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <input type="text"
                       ...
                       value="<?= htmlspecialchars($oldInput['title'] ?? $quiz['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                       required>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea id="description"
                          name="description"
                          class="form-control"
                          rows="3"><?= htmlspecialchars($oldInput['description'] ?? $quiz['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>
            ...
            <div class="row">
                    ...
                    <select id="topic_id"
                        ...
                        <?php foreach ($topics as $topic): ?>
                            <option value="<?= (int) $topic['id'] ?>"
                                <?= ((int) ($oldInput['topic_id'] ?? $quiz['topic_id'] ?? 0)) === (int) $topic['id'] ? 'selected' : '' ?>>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number"
                           ...
                           class="form-control"
                           min="0"
                           value="<?= htmlspecialchars((string) ($oldInput['time_limit_min'] ?? $quiz['time_limit_min'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number"
                           ...
                           min="0"
                           max="100"
                           value="<?= htmlspecialchars((string) ($oldInput['pass_percentage'] ?? $quiz['pass_percentage'] ?? 50), ENT_QUOTES, 'UTF-8') ?>">
            ...
            <div class="form-check">
                <input type="checkbox"
                       ...
                       value="1"
                       <?= !empty($oldInput['is_active'] ?? $quiz['is_active'] ?? false) ? 'checked' : '' ?>>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            <!-- Build a lookup of assigned question IDs -->
            <?php
                $assignedIds = [];
                if (isset($quiz['questions']) && is_array($quiz['questions'])) {
                    foreach ($quiz['questions'] as $aq) {
                        $assignedIds[(int) $aq['id']] = (int) ($aq['sort_order'] ?? 0);
                    }
                }
            ?>
            ...
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-sm table-hover mb-0" id="questionBankTable">
                        ...
                        <tbody>
                            <?php
                                // Sort assigned questions first, then remaining
                                $sortedQuestions = [];
                                $remaining = [];

                                foreach ($allQuestions as $q) {
                                    $qid = (int) $q['id'];
                                    if (isset($assignedIds[$qid])) {
                                        $q['_sort_order'] = $assignedIds[$qid];
                                        $sortedQuestions[] = $q;
                                    } else {
                                        $q['_sort_order'] = PHP_INT_MAX;
                                        $remaining[] = $q;
                                    }
                                }

                                usort($sortedQuestions, fn($a, $b) => $a['_sort_order'] - $b['_sort_order']);
                                $allSorted = array_merge($sortedQuestions, $remaining);
                            ?>

                            <?php foreach ($allSorted as $question): ?>
                                <?php
                                    $qid = (int) $question['id'];
                                    $typeLabels = [
                                    ...
                                    $isAssigned = isset($assignedIds[$qid]);
                                ?>
                                <tr class="question-row<?= $isAssigned ? ' table-primary' : '' ?>"
                                    data-topic-id="<?= (int) $question['topic_id'] ?>"
                                    ...
                                    <td>
                                        <input type="checkbox"
                                               ...
                                               data-question-id="<?= $qid ?>"
                                               <?= $isAssigned ? 'checked' : '' ?>
                                               <?= isset($oldInput['question_ids']) && in_array($qid, (array) $oldInput['question_ids']) ? 'checked' : '' ?>>
                                    </td>
                                    <td>
